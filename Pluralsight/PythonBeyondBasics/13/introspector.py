...
def print_table(rows_of_columns, *headers):
    num_columns: int = len(rows_of_columns[0])
    num_headers: int = len(headers)
    if len(headers) != num_columns:
        raise TypeError(f"Expected {num_columns} header args but got {num_headers}")
...
def dump(obj):
    ...
    all_attr_names = all_attr_names - method_names
    attr_names_and_values = [(name, reprlib.repr(getattr(obj, name))) for name in all_attr_names]
